    ...
    .then(function(accounts) {
      ...
      return Promise.all(
        accounts.map(function(entry) {
          ...
          debug('GOING TO DELETE ', { serviceName, accountName });
          return keytar
...
SecureBackend.prototype.remove = function(model, options, done) {
  ...
  console.log('SECUREBACKEND REMOVE', accountName, serviceName);
  keytar.getPassword(serviceName, accountName).then(function(r) {
    if (!r) {
      warn('NO PASSWORD TO DELETE', { serviceName, accountName });
      return done();
    }
    keytar
      .deletePassword(serviceName, accountName)
...
SecureBackend.prototype.find = function(collection, options, done) {
  ...
  keytar
    ...
    .then(credentials => {
      debug('credentials found in namespace', {
        credentials: credentials,
        namespace: this.namespace
      });
      console.warn('CREDENTIALS', JSON.stringify(credentials));
      if (
        isNaN(credentials.length) ||
        credentials.length === 0 ||
        !credentials
      ) {
        console.warn('NO CREDENTIALS TO FIND');
        return done();
      }
