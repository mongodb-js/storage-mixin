...
if (_.isEmpty(fs)) {
  ...
  try {
    ...
    throw new Error(
      'browser context, `fs` module not available for disk storage'
    );
  }
}
...
DiskBackend.prototype._getId = function(modelOrFilename) {
  var id =
    typeof modelOrFilename === 'string'
      ? modelOrFilename
      : modelOrFilename.getId();
...
DiskBackend.prototype.findOne = function(model, options, done) {
  ...
  fs.exists(file, function(exists) {
    ...
    fs.readFile(file, 'utf8', function(err, content) {
      ...
      debug('parsing from disk', content);
      try {
...
DiskBackend.prototype.find = function(collection, options, done) {
  ...
  fs.readdir(this.path, function(err, files) {
    ...
    if (filtered.length === 0) {
      debug('filtered is empty');
      return done();
    }

    var tasks = filtered.map(function(file) {
      return self.findOne.bind(
        self,
